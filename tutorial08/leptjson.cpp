    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < lhs->u.o.size; i++) {
                ...
                if ((pos = lept_find_object_index(rhs, m->k, m->klen)) == LEPT_PARSE_MISS_KEY)
                    return 0;
                if (!lept_is_equal(&m->v, &rhs->u.o.m[pos].v))
                    return 0;
            }
            return 1;
        default:
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    ...
    /* \todo */
    if (count) {
        for (int i = index + count; i < v->u.a.size; i++)
            lept_move(&v->u.a.e[i - count], &v->u.a.e[i]);
        for (int i = 0; i < count; i++)
            lept_free(&v->u.a.e[v->u.a.size - i - 1]);
        v->u.a.size -= count;
    }
}
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    ...
    memcpy(v->u.o.m[v->u.o.size].k = (char*)malloc(klen + 1), key, klen + 1);
...
void lept_remove_object_value(lept_value* v, size_t index) {
    ...
    /* \todo */
    size_t pos;
    for (int i = index + 1; i < v->u.o.size; i++) {
        free(v->u.o.m[i - 1].k);
        memcpy(v->u.o.m[i - 1].k = (char*)malloc(v->u.o.m[i].klen + 1), v->u.o.m[i].k, v->u.o.m[i].klen + 1);
