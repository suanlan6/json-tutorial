...
static void test_parse_array() {
    ...
    for (int i = 0;i < 4;i++) {
        lept_value *k = lept_get_array_element(&v, i);
        for (int j = 0;j < lept_get_array_size(k);j++) {
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(lept_get_array_element(k, j)));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(lept_get_array_element(k, j)));
        }
    }
    lept_free(&v);
}
